...
export class BSONMap extends Map<string, any> {
    ...
    constructor(buffer: ArrayBuffer) {
        ...
        const { entries } = BSONMap.parse(new Uint8Array(buffer));
        super(entries);
    }
    ...
    private static parse(sequence: Uint8Array, offset = 0): { entries: Entries, documentByteLength: number } {
        ...
        while (readable && index < documentSize) {
            ...
            switch (type) {
                ...
                case TYPE.DOCUMENT: {
                    const { entries, documentByteLength } = BSONMap.parse(sequence.subarray(index), index)
            ...
            entries.push([key, { value, position: position++, type: TYPE.getTypeName(type) }])
        }
    ...
    private static mapToArray(map: Map<string, any>) {
        const list = []
